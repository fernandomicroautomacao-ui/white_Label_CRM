...
(function () {
    ...
    function selecionarEmpresaCentral(id) {
        ...
        detail.innerHTML = `<div class="company-detail-header"><div><span class="quick-start-kicker">Ficha comercial</span><h3>${text(lead.empresa)}</h3><p class="text-muted">${text(lead.cnpj || 'CNPJ não informado')} · ${text(lead.cidade || 'Localidade não informada')}</p></div><div style="display:flex;gap:6px;"><button class="btn btn-outline btn-sm" onclick="abrirMergulhoProfundoLead('${text(lead.id)}')">🔍 Mergulho</button><button class="btn btn-primary btn-sm" onclick="abrirModalLead('${text(lead.id)}')">Editar</button></div></div><div class="company-detail-grid"><div><span>Etapa</span><strong>${text(lead.etapa || '—')}</strong></div><div><span>Classificação</span><strong>${text(classifNome)}</strong></div><div><span>Potencial</span><strong>${text(lead.potencial || '—')}</strong></div><div><span>Valor</span><strong>${money(lead.valor)}</strong></div></div><div class="company-detail-contact"><a href="tel:${text(lead.telefone)}">${text(lead.telefone || 'Sem telefone')}</a><a href="mailto:${text(lead.email)}">${text(lead.email || 'Sem e-mail')}</a><span>${text(lead.decisor || 'Decisor não informado')}</span></div><div class="ai-recommendation"><strong>Recomendação:</strong> ${recommendation}</div><div class="company-history"><div class="card-header"><h3>Últimas atividades</h3><button class="btn btn-outline btn-sm" onclick="abrirAtividade('${text(lead.id)}')">Registrar atividade</button></div>${history.length ? history.map(item => `<div class="history-row"><span>${text(item.data)} ${text(item.hora)}</span><strong>${text(item.tipo)}</strong><p>${text(item.descricao)}</p></div>`).join('') : '<div class="empty-state compact"><p>Nenhuma atividade registrada.</p></div>'}</div>`;
    ...
    function exportarBackupCompleto() {
        const payload = { versao: 1, exportadoEm: new Date().toISOString(), leads: safeLeads(), localStorage: { commission: localStorage.getItem(STORAGE_KEYS.commission), cadences: localStorage.getItem(STORAGE_KEYS.cadences), metas: localStorage.getItem('ploomesMetasV5'), checkpointsOrcamento: localStorage.getItem('crm_checkpoints_orcamento_dias') } };
    ...
    function restaurarBackupCompleto(event) {
        ...
        const reader = new FileReader(); reader.onload = () => { try { const payload = JSON.parse(reader.result); if (!payload || !Array.isArray(payload.leads)) throw new Error('Formato inválido'); if (!confirm('Restaurar este backup substituirá os leads locais atuais. Deseja continuar?')) return; leads = payload.leads; if (payload.localStorage?.commission) localStorage.setItem(STORAGE_KEYS.commission, payload.localStorage.commission); if (payload.localStorage?.cadences) localStorage.setItem(STORAGE_KEYS.cadences, payload.localStorage.cadences); if (payload.localStorage?.checkpointsOrcamento) localStorage.setItem('crm_checkpoints_orcamento_dias', payload.localStorage.checkpointsOrcamento); if (typeof salvarDados === 'function') salvarDados(); if (typeof renderizarAll === 'function') renderizarAll(); registrarAuditoriaLocal('Restauração de backup', 'sistema', '', 'Backup JSON restaurado'); if (typeof showToast === 'function') showToast('Backup restaurado.'); } catch { if (typeof showToast === 'function') showToast('Arquivo de backup inválido.', 'error'); } finally { event.target.value = ''; } }; reader.readAsText(file);
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        renderizarFinanceiro(); renderizarAuditoriaLocal();
