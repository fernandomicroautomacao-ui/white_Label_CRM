            ...
            <div class="pipeline-column" data-etapa="${etapa.id}"
                ...
                ${items.map(lead => {
                    ...
                    <div class="pipeline-card ${lead.etapa} ${temAlertaCheckpoint ? 'com-alerta-checkpoint' : ''}"
                        ...
                        ${(Number(lead.landingPageViews) > 0) ? `<div class="card-badge" onclick="event.stopPropagation();typeof abrirModalRelatorioVisualizacoesLP === 'function' ? abrirModalRelatorioVisualizacoesLP('${lead.id}') : (typeof abrirModalLandingPageLead === 'function' ? abrirModalLandingPageLead('${lead.id}') : null)" title="Cliente visualizou a Proposta/Landing Page ${lead.landingPageViews}x (Último acesso: ${lead.landingPageUltimoAcesso ? new Date(lead.landingPageUltimoAcesso).toLocaleString('pt-BR') : 'recente'}). Clique para auditar." style="cursor:pointer;background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;font-weight:700;font-size:10px;padding:2px 7px;border-radius:4px;display:inline-flex;align-items:center;gap:3px;margin-top:3px;">👁️ ${lead.landingPageViews} visualizaç${Number(lead.landingPageViews) > 1 ? 'ões' : 'ão'}</div>` : ''}
                                ...
                                ` : `
                                    ...
                                    <div class="card-menu-wrap">
                                        ...
                                        <div class="card-menu" id="cardMenu-${lead.id}">
                                            <button onclick="fecharCardMenus();abrirModalLandingPageLead('${lead.id}')">🌐 Landing Page do Cliente ${Number(lead.landingPageViews) > 0 ? `(${lead.landingPageViews} views)` : ''}</button>
